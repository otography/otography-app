...
export class FirebaseTokenVerifier {
  ...
  constructor(
    ...
    this.signatureVerifier = PublicKeySignatureVerifier.withCertificateUrl(clientCertUrl);
  ...
  private async safeDecode(jwtToken: string): Promise<DecodedToken> {
    try {
      ...
      throw new FirebaseAuthError(AuthClientErrorCode.INTERNAL_ERROR, jwtErr.message ?? "");
    }
  }
  ...
  private verifyContent(
    ...
    if (!isEmulator && typeof header.kid === "undefined") {
      ...
      const isLegacyCustomToken =
        header.alg === "HS256" && payload.v === 0 && "d" in payload && "uid" in payload.d;
      ...
      if (typeof payload.sub !== "string") {
        errorMessage =
          `${this.tokenInfo.jwtName} has no "sub" (subject) claim.` + verifyJwtTokenDocsMessage;
      } else if (payload.sub === "") {
        errorMessage =
          `${this.tokenInfo.jwtName} has an empty "sub" (subject) claim.` +
          verifyJwtTokenDocsMessage;
      } else if (payload.sub.length > 128) {
        errorMessage =
          `${this.tokenInfo.jwtName} has a "sub" (subject) claim longer than 128 characters.` +
          verifyJwtTokenDocsMessage;
      }
    }
    if (errorMessage) {
  ...
  private mapJwtErrorToAuthError(error: JwtError): Error {
    ...
    if (error.code === JwtErrorCode.TOKEN_EXPIRED) {
      ...
      const errorMessage =
        `${this.tokenInfo.jwtName} has invalid signature.` + verifyJwtTokenDocsMessage;
