...
export class UserImportBuilder {
	 ...
	 */
	public buildResponse(failedUploads: Array<{ index: number; message: string }>): UserImportResult {
		...
		failedUploads.forEach((failedUpload) => {
			importResult.errors.push({
				...
				index: this.indexMap[failedUpload.index] ?? failedUpload.index,
				error: new FirebaseAuthError(AuthClientErrorCode.INVALID_USER_IMPORT, failedUpload.message),
			});
		});
		...
		users.forEach((user, index) => {
			try {
				...
				this.userImportResultErrors.push({
					index,
					error: error as FirebaseAuthError,
				});
			}
		});
