...
const AUTH_FACTOR_SERVER_TO_CLIENT_TYPE: { [key: string]: AuthFactorType } = Object.keys(
	AUTH_FACTOR_CLIENT_TO_SERVER_TYPE,
	...
	public static buildServerRequest(options: MultiFactorConfig): MultiFactorAuthServerConfig {
		...
		if (Object.prototype.hasOwnProperty.call(options, "factorIds")) {
			(options.factorIds || []).forEach((factorId) => {
				...
				const serverType = AUTH_FACTOR_CLIENT_TO_SERVER_TYPE[factorId as string];
				if (serverType) {
					request.enabledProviders.push(serverType);
				}
			});
	...
	public static validate(options: MultiFactorConfig): void {
		...
		if (typeof options.providerConfigs !== "undefined") {
			...
			options.providerConfigs.forEach((multiFactorProviderConfig) => {
				if (
					typeof multiFactorProviderConfig === "undefined" ||
					!validator.isObject(multiFactorProviderConfig)
				) {
					throw new FirebaseAuthError(
						AuthClientErrorCode.INVALID_CONFIG,
				...
				if (
					typeof multiFactorProviderConfig.state === "undefined" ||
	...
	constructor(response: MultiFactorAuthServerConfig) {
		...
		(response.providerConfigs || []).forEach((providerConfig) => {
			if (typeof providerConfig !== "undefined") {
				if (
					typeof providerConfig.state === "undefined" ||
 ...
 */
export function validateTestPhoneNumbers(testPhoneNumbers: {
	...
	for (const phoneNumber in testPhoneNumbers) {
		...
		// Validate code.
		if (
			!validator.isString(testPhoneNumbers[phoneNumber]) ||
...
export class EmailSignInConfig implements EmailSignInProviderConfig {
	...
	public static buildServerRequest(
		options: EmailSignInProviderConfig,
	): EmailSignInConfigServerRequest {
	...
	private static validate(options: EmailSignInProviderConfig): void {
		...
		if (
			typeof options.passwordRequired !== "undefined" &&
...
export class SAMLConfig implements SAMLAuthProviderConfig {
	...
	): SAMLConfigServerRequest | null {
		const makeRequest =
			validator.isNonNullObject(options) && (options.providerId || ignoreMissingFields);
		if (!makeRequest) {
			return null;
		}
	...
	public static getProviderIdFromResourceName(resourceName: string): string | null {
		...
		return matchProviderRes[1] ?? null;
	}
	...
	public static validate(
		...
		if (
			!(ignoreMissingFields && typeof options.idpEntityId === "undefined") &&
		...
		if (
			!(ignoreMissingFields && typeof options.ssoURL === "undefined") &&
		...
		if (
			!(ignoreMissingFields && typeof options.rpEntityId === "undefined") &&
		...
		if (
			!(ignoreMissingFields && typeof options.callbackURL === "undefined") &&
		...
		if (
			!(ignoreMissingFields && typeof options.x509Certificates === "undefined") &&
		...
		if (
			typeof (options as any).enableRequestSigning !== "undefined" &&
	...
	constructor(response: SAMLConfigServerResponse) {
		if (
			...
			!response.name ||
			!(
				validator.isString(response.name) && SAMLConfig.getProviderIdFromResourceName(response.name)
			)
		) {
			throw new FirebaseAuthError(
				AuthClientErrorCode.INTERNAL_ERROR,
...
export class OIDCConfig implements OIDCAuthProviderConfig {
	...
	public static validate(
		...
		if (
			!(ignoreMissingFields && typeof options.clientId === "undefined") &&
		...
		if (
			!(ignoreMissingFields && typeof options.issuer === "undefined") &&
		...
		if (
			validator.isNonNullObject(options.responseType) &&
	...
	constructor(response: OIDCConfigServerResponse) {
		if (
			...
			!response.name ||
			!(
				validator.isString(response.name) && OIDCConfig.getProviderIdFromResourceName(response.name)
			)
		) {
			throw new FirebaseAuthError(
				AuthClientErrorCode.INTERNAL_ERROR,
...
export class SmsRegionsAuthConfig {
	public static validate(options: SmsRegionConfig): void {
		...
		if (
			typeof options.allowByDefault !== "undefined" &&
			typeof options.allowlistOnly !== "undefined"
		) {
			throw new FirebaseAuthError(
				AuthClientErrorCode.INVALID_CONFIG,
		...
		if (typeof options.allowByDefault !== "undefined") {
			...
			if (
				typeof options.allowByDefault.disallowedRegions !== "undefined" &&
		...
		if (typeof options.allowlistOnly !== "undefined") {
			...
			if (
				typeof options.allowlistOnly.allowedRegions !== "undefined" &&
...
export class RecaptchaAuthConfig implements RecaptchaConfig {
	...
	public static validate(options: RecaptchaConfig): void {
		...
		if (typeof options.phoneEnforcementState !== "undefined") {
			...
			if (
				options.phoneEnforcementState !== "OFF" &&
...
export class PasswordPolicyAuthConfig implements PasswordPolicyConfig {
		...
		if (Object.prototype.hasOwnProperty.call(options, "constraints")) {
			if (options) {
				...
				if (options.constraints?.requireNonAlphanumeric !== undefined) {
					constraintsRequest.containsNonAlphanumericCharacter =
						options.constraints.requireNonAlphanumeric;
				}
	...
	public static validate(options: PasswordPolicyConfig): void {
		...
		// Validate content.
		if (
			typeof options.enforcementState === "undefined" ||
			!(options.enforcementState === "ENFORCE" || options.enforcementState === "OFF")
		) {
			throw new FirebaseAuthError(
				AuthClientErrorCode.INVALID_CONFIG,
		...
		if (typeof options.constraints !== "undefined") {
			if (
				...
				!validator.isNonNullObject(options.constraints)
			) {
				throw new FirebaseAuthError(
					AuthClientErrorCode.INVALID_CONFIG,
			...
			if (
				typeof options.constraints.requireUppercase !== "undefined" &&
			...
			if (
				typeof options.constraints.requireLowercase !== "undefined" &&
			...
			if (
				typeof options.constraints.requireNonAlphanumeric !== "undefined" &&
				!validator.isBoolean(options.constraints.requireNonAlphanumeric)
			) {
				throw new FirebaseAuthError(
					AuthClientErrorCode.INVALID_CONFIG,
					'"PasswordPolicyConfig.constraints.requireNonAlphanumeric"' + " must be a boolean.",
				);
			}
			if (
				typeof options.constraints.requireNumeric !== "undefined" &&
			...
			if (typeof options.constraints.maxLength === "undefined") {
				...
				if (
					!(
						options.constraints.maxLength >= options.constraints.minLength &&
						options.constraints.maxLength <= 4096
					)
				) {
					throw new FirebaseAuthError(
			...
			(response.passwordPolicyVersions || []).forEach((policyVersion) => {
				constraintsResponse = {
					...
					requireNonAlphanumeric:
						policyVersion.customStrengthOptions?.containsNonAlphanumericCharacter,
...
export class EmailPrivacyAuthConfig {
	public static validate(options: EmailPrivacyConfig): void {
		...
		if (
			typeof options.enableImprovedEmailPrivacy !== "undefined" &&
