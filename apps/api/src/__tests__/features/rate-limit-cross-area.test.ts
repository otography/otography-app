...
describe("VAL-CROSS-001: 全エンドポイントの429レスポンスが一貫したエラー形式を持つ", () => {
  ...
  it("sign-in, コンテンツ作成, いいね の429レスポンスが同じ body shape を持つ", async () => {
    ...
    const expectedShape = { message: "Too many requests. Please try again later." };

    expect(signinBody).toEqual(expectedShape);
    expect(contentBody).toEqual(expectedShape);
    expect(likeBody).toEqual(expectedShape);
  });
});
